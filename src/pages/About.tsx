...
const About = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground font-sans">
      ...
      {/* Hero Section */}
      <motion.section
        className="relative bg-gradient-to-br from-accent to-background text-center py-24 px-6 md:px-10"
        initial="hidden"
        animate="visible"
        variants={fadeIn}
      >
        <Hotel className="mx-auto text-primary mb-4" size={48} />
        <h1 className="text-5xl font-extrabold text-foreground mb-4">{t('Welcome to HotelHub')} </h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          {t('Your ultimate destination for finding quality stays across the globe. We make your travel dreams a reality')}
        </p>
      </motion.section>
      ...
      <main className="flex-grow px-6 md:px-10 max-w-6xl mx-auto w-full py-16 space-y-24">
        ...
        >
          ...
          <h2 className="text-3xl font-semibold text-foreground mb-4">{t('Our Story')} </h2>
        ...
        <motion.section
          ...
          variants={fadeIn}
        >
          <h2 className="text-3xl font-semibold text-center text-foreground mb-10">{t('Our Core Values')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <motion.div
                key={i}
                className="bg-card p-6 rounded-2xl shadow-md text-center hover:shadow-lg transition duration-300"
                custom={i}
                variants={fadeIn}
              >
                {value.icon}
        ...
        {/* CTA */}
        <motion.section
          className="bg-primary text-white text-primary-foreground rounded-2xl py-10 px-6 text-center shadow-lg"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
          variants={fadeIn}
        >
          ...
          <a
            href="/hotels"
            className="inline-block px-6 py-3 bg-background text-foreground font-semibold rounded-full hover:bg-accent transition"
          >
            {t('Browse Hotels')}
          </a>
        </motion.section>
